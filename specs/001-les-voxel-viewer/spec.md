...
**Created**: 2026-02-12  
**Status**: Draft  
**Input**: User description: "平文形式のボクセルデータを読み込み、VSCode内でレンダリングする拡張機能を作成したい。ファイルツリーで *.leS を直接開く、*.leS ファイルを開いたエディタからビュアーを開く、コマンドパレットでビュアーを開きleSファイルをドラッグ・アンド・ドロップ、の方法でビュアーを開きたい。tmp/ にシェーダーとreact実装があり、最大 1024x1024x1024 程度のボクセルを想定。"
...
- ヘッダのサイズ指定と実データ数が一致しない場合、厳密に検証してエラーを表示する（FR-005, FR-011で対応）
- .leS のサイズが上限（1024x1024x1024）を超える場合、ヘッダ検証時に即座に拒否する（FR-008で対応）
- 値が許容範囲外（0-255の整数以外）の場合、パース時に検証してエラーを表示する（FR-015で対応）
- ボクセルが全て空（値が0）の場合、正常に読み込み空のグリッドを表示する
- 低性能環境で描画負荷が高い場合
- 描画環境（WebGL等）が利用できない場合、明確なエラーメッセージを表示してビューアーを開かない（FR-016で対応）
...
- **FR-008**: システムはヘッダ読み込み時にサイズを検証し、上限（最大 1024x1024x1024）を超える場合は読み込みを拒否して明示的なエラーを表示すること
...
- **FR-015**: システムはボクセル値のパース時に0-255の整数範囲を検証し、範囲外の値が存在する場合はエラーを表示すること
- **FR-016**: システムは描画環境（WebGL等）が利用できない場合、明確なエラーメッセージを表示してビューアーを開かないこと
...
- Q: ヘッダのサイズ指定と実データ数が一致しない場合はどうするか? → A: 厳密に検証し、一致しない場合はエラーを表示する
- Q: .leS のサイズが上限を超える場合の扱いは? → A: ファイル読み込み前にヘッダで判定し、即座に拒否する
- Q: 値が許容範囲外（例: 負値や大きすぎる値）の場合の扱いは? → A: パース時に数値範囲を検証し、範囲外があればエラーを表示する
- Q: 描画環境が利用できない場合の扱いは? → A: 明確なエラーメッセージを表示し、ビューアーを開かない
- Q: ボクセルが全て空（値が0）の場合の扱いは? → A: 正常に読み込み、空のグリッドを表示する
